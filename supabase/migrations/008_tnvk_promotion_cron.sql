...
BEGIN
  ...
  LOOP
    -- Số tháng từ ngay_huong_bac đến hiện tại (làm tròn xuống)
    v_months := (
    ...
    -- Chỉ update khi giá trị khác → tránh trigger audit nổ rỗng.
...
--    Trả về danh sách NV (KHÔNG ở bậc cuối) có ngày nâng bậc tiếp theo
--    rơi vào [hôm nay; hôm nay + days_ahead]. Mặc định 30 ngày.
--    Ngày nâng bậc = ngay_huong_bac + chu_ky_nang_bac_nam (RULE-02).
-- =====================================================================
...
--    Wrapper được pg_cron gọi đầu mỗi tháng. Gọi cả 2 hàm trên rồi ghi
--    tổng kết vào activity_logs (action='system.tnvk_promotion_job') để
--    admin_he_thong xem được trong trang /logs.
--    Trả về 1 string mô tả ngắn (dùng cho cron.job_run_details).
-- =====================================================================
...
-- =====================================================================
-- 4. CRON JOB — đăng ký 00:30 UTC ngày 1 hàng tháng (~07:30 VN ngày 1)
--    Idempotent: drop job cũ cùng tên trước khi schedule.
-- =====================================================================
DO $$
DECLARE
  jid bigint;
...
--
--   -- 2) Chạy thủ công ngay (không đợi đến ngày 1):
--   SELECT public.run_monthly_tnvk_promotion_job();
--
...
--
-- ROLLBACK (gỡ job, giữ functions để dùng manual):
--
